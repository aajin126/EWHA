...
#include <learnopengl/camera.h> // 카메라 헤더!!!
...
void DrawFingerTip(glm::mat4 model); // wrist에 대해 relative
...
void myDisplay()
{
	...
	DrawGroundPlane(model); // 바닥 그리기

	// ADD YOUR ROBOT RENDERING STUFF HERE//////////////////////////////////////
	
	//case0 - ok
	model = glm::translate(model, glm::vec3(BaseTransX, 0.0f, BaseTransZ)); //  model 행렬에 대해 이동 변환을 적용
	...
	model = glm::rotate(model, glm::radians(ShoulderAng), glm::vec3(0.0f, 0.0f, 1.0f)); //z축 기준 회전
	DrawArmSegment(model);

	//Teapot
	objectXform = glm::translate(objectXform, glm::vec3(0.0f, 0.4f, 0.0f));

	objectXform = glm::rotate(objectXform, glm::radians(ShoulderAng), glm::vec3(0.0f, 0.0f, 1.0f)); // ShoulderAng 변수에 비례하여 Z 축을 중심으로 회전

	objectXform = glm::rotate(objectXform, glm::radians(TeapotShoulderAng), glm::vec3(0.0f, 0.0f, 1.0f)); //  TeapotShoulderAng 변수에 비례하여 Z 축 주위의 추가 회전
	...
	model = glm::rotate(model, glm::radians(ElbowAng), glm::vec3(0.0f, 0.0f, 1.0f)); //z축 기준 회전
	DrawArmSegment(model);
	...
	//WristAng
	model = glm::translate(model, glm::vec3(0.0f, 0.5f, 0.0f));
	model = glm::rotate(model, glm::radians(WristAng), glm::vec3(0.0f, 0.0f, 1.0f));//z축 기준 회전, 회전각 : 0.02
	//WristTwistAng
	model = glm::rotate(model, glm::radians(WristTwistAng), glm::vec3(0.0f, 1.0f, 0.0f));//y축 기준 회전, 회전각 : 0.02
	DrawWrist(model);
	...
	model_l = glm::rotate(model_l, glm::radians(FingerAng1), glm::vec3(0.0f, 0.0f, 1.0f));//z축 기준 회전
	glm::mat4 model_left = model_l;
	DrawFingerBase(model_l);

	//FingerAng1 - right
	model_r = glm::rotate(model_r, glm::radians(-FingerAng1), glm::vec3(0.0f, 0.0f, 1.0f));//z축 기준 회전
	...
	model_left = glm::rotate(model_left, glm::radians(FingerAng2), glm::vec3(0.0f, 0.0f, 1.0f));//z축 기준 회전, 회전각 : 0.02
...
int main()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		//glm::mat4 view = camera.GetViewMatrix(); // view 설정
...
void processInput(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	...
	if (key >= GLFW_KEY_1 && key <= GLFW_KEY_5 && action == GLFW_PRESS) //어떤 키보드 키를 눌렀는지 저장
...
void mouse_callback(GLFWwindow* window, double xpos, double ypos)
{
	...
	if (LeftButtonDown) // 마우스 왼쪽 버튼을 클릭할 때마다 변하는 -> 이 변수값을 modeling transformation할 때 적절히 사용!
	{
		switch (RobotControl)
		{
